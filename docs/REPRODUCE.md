...
1) 重要脚本（位于 `code/`）
- `../scripts/run_lastfm_300x3.sh` — LastFM baseline 与 SimGCL（各 3 个 seed）的批量运行脚本。
- `../scripts/run_gowalla_combo1_300x3.sh` — Gowalla 上的 `gowalla_best` 组合（layer_agg=learnable + layer_w regs + SimGCL）3 seed 批量脚本。
- 其它可能相关脚本：
  - `../scripts/run_learnable_l2_300_3seeds.sh`
  - `../scripts/run_l2_resume_seed2021_seed2022.sh`
  - `../scripts/run_tau_screen_150.sh`, `../scripts/run_tau_screen_resume.sh`
...
- 直接使用脚本（在仓库根目录执行）
```bash
bash ../scripts/run_lastfm_300x3.sh
bash ../scripts/run_gowalla_combo1_300x3.sh
```
